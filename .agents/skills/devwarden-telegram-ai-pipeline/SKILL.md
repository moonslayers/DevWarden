...
- `app/Services/Telegram/TelegramClient.php` — thin wrapper around `telegram-bot/api` with an injectable Guzzle client. `normalizeUpdate()` parses `message` and `edited_message` (via `Update::getEditedMessage()`): for a message it always exposes `message_id` and sets `edit=true` only for edits — shape `{update_id, chat_id?, message_id?, text?, photo?, edit?}`; an update with NO message/edited_message (e.g. a leftover `callback_query`) returns just `['update_id' => ...]` — there is NO callback branch anymore (the inline-keyboard pipeline was removed). For a photo message `text` becomes the caption (when present) and `photo` = the LARGEST-area `PhotoSize.file_id`. `sendMessage(int|string $chatId, string $text, ?string $parseMode = null)` adds `parse_mode` only when non-null — no `reply_markup` parameter (removed with the keyboard pipeline). Also exposes `getFile($fileId)` (resolve download metadata) and `downloadFile($filePath, $destinationPath)` (writes the Telegram-hosted bytes locally).
...
- `app/Console/Commands/PollTelegramUpdates.php` (`telegram:poll`) — long-poll + allowlist + offset persistence; buffers authorized message/photo updates via `TelegramMessageBuffer` (a leftover `callback_query` update has no message → dropped like any non-text update, offset still advances).
...
## OpenCode question notifications are plain text (no inline keyboard)

The opencode watcher's "esperando tu respuesta (tiene preguntas)" notifications are PLAIN TEXT (numbered `Pregunta N:` + lettered `(a) label` options, no button markup). The inline-button callback pipeline was REMOVED by user decision: the callback job, the button-payload contract and the callback routing in `normalizeUpdate`/`PollTelegramUpdates` are all gone, and updates without a message are dropped with the offset still advancing. Pending opencode questions are answered through the bot via `OpencodeAskTool` (the `<active_opencode_sessions>` prompt block carries the `session_id` + question labels). The watcher and `questionOptions()` details live in the **devwarden-opencode-orchestration** skill.
...
## Diagnóstico: bot no responde (worker muerto / batch atascado)

Real incident. Symptom: the bot does NOT answer ANY message (no reply, no error in laravel.log, no placeholder). Unlike the config-corrupt case above there is no `sendMessage failed` — the pipeline never runs.

- **Causa 1 — OOM-kill del queue:work**: `composer run dev:full` lanza 3 procesos (serve:8012 + schedule:work + queue:work con `-d extension=ffi` para ONNX). El worker puede morir por OOM-kill del kernel bajo saturación de RAM (Committed_AS > CommitLimit, swap lleno, opencode TUIs a 1.3-1.8GB). OOM-kill = SIGKILL → sin error en laravel.log, sin core dump de php (los `core.opencode.*` de `/var/lib/systemd/coredump` son de opencode, no de php). Verificar con `ps -eo pid,cwd,cmd | rg 'artisan queue:work'` + `pwdx <pid>` (el de s2c tiene cwd distinto). Mitigación: liberar RAM antes de `dev:full`.
- **Causa 2 — batch atascado**: un worker que muere a mitad de un drain deja `telegram_chat_batches.processing_at` puesto. `scheduleIfNeeded()` SOLO re-arma batch para chats que reciben un update nuevo (PollTelegramUpdates llama `scheduleIfNeeded` solo para `$affectedChats`) y solo reclama `processing_at` stale (>15 min, `STALE_THRESHOLD`). Resultado: hasta 15 min de silencio + requiere otro mensaje. Unblock manual: `php artisan tinker --execute 'App\Jobs\ProcessTelegramPendingBatch::dispatch(<chat_id>);'`.
- **Causa 3 — timeout del drain**: `ProcessTelegramPendingBatch` ahora define `$timeout = 300` (ver arriba). Sin él, el worker corre sin `--timeout` → default 60s de Laravel → IA lenta (>60s) mata el job con `TimeoutExceededException` → mismo batch atascado → mismo síntoma.
- **Ops**: el monitor (`opencode:monitor`) corre un proceso `artisan` nuevo cada tick → los cambios en el watcher son LIVE sin reiniciar dev:full; solo el `queue:work` (long-running) requiere restart de dev:full para cargar clases nuevas.
...
- Buffer/batch coverage: `tests/Feature/Services/TelegramMessageBufferTest.php` (upsert on edit, `storeMessage` persisting `photo_file_id`, `scheduleIfNeeded` dedup + stale reclaim, begin/endProcessing), `tests/Feature/Jobs/ProcessTelegramPendingBatchTest.php` (text coalescing into one AI turn, one AI turn per photo message with the downloaded image, per-turn placeholder, friendly-error skips `CaptureBotMemory`, photo cleanup on success/failure/partial download, extension fallback to jpg, missing owner; ALSO asserts `tries=1` and `$timeout=300` on the job — "allows the AI generation an explicit generous timeout so slow providers do not kill the drain"), `tests/Feature/Jobs/ProcessTelegramPendingBatchPlaceholderTest.php` (placeholder ordering: placeholder before the reply), `tests/Feature/Console/PollTelegramUpdatesTest.php` (storeMessage + scheduleIfNeeded wiring and offset persistence; a leftover `callback_query` update is DISCARDED without buffering — no job is pushed — while the offset still advances).
