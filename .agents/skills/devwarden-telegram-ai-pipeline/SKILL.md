...
- `app/Services/Telegram/TelegramClient.php` — thin wrapper around `telegram-bot/api` with an injectable Guzzle client. `normalizeUpdate()` parses both `message` and `edited_message` (via `Update::getEditedMessage()`), always exposing `message_id` when a message exists and setting `edit=true` only for edits — shape `{update_id, chat_id?, message_id?, text?, photo?, edit?}`. For a photo message `text` becomes the caption (when present) and `photo` = the LARGEST-area `PhotoSize.file_id`. Also exposes `getFile($fileId)` (resolve download metadata) and `downloadFile($filePath, $destinationPath)` (writes the Telegram-hosted bytes locally).
