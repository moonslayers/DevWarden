...
## BotAgent pending-question block injects session_id + question details via questionOptions()
resolvePendingQuestionDetails() reads sessionState()['last_turn_tool'] === 'question' per session — the same live signal the watcher uses for notifyQuestionTurn — and for those sessions (normally 1-2, never the whole block) ALSO calls store->questionOptions($id) (guarded per-session via resolveQuestionOptions(); [] on failure) keyed by session id. formatActiveSessionsBlock(array $sessions, array $workingIds, array $questionDetails) shows the awaiting-question line plus '  session_id: ses_xxx', '  Pending questions:', '  Q1: <text>' and '    - <option label>' lines — giving the model the exact labels to reply with via OpencodeAskTool (blocking=true). A pending session with [] options degrades to the status-only line. Never call questionOptions() for the whole block — only for flagged sessions.
