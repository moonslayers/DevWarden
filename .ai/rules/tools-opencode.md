...
## ReadSessionTool reads parts via recentParts (DB-only, no MCP)
ReadSessionTool (extends OpencodeSessionTool, store as 2nd ctor arg like SearchSessionsTool) reads the last/first messages, tool calls and sub-agents of an opencode session straight from the opencode SQLite via OpencodeSessionStore::recentParts() — it never calls the MCP manager (DB-only). Registered in BotAgent::tools() right after SearchSessionsTool. It shows role/text, tool calls (tool+status+input/output) and sub-agents (agent_name + sub_session_id), plus status (working/idle, esperando respuesta when last_turn_tool=question) and the dismissed flag (OpencodeSessionDismissal) so the bot can decide to call MarkSessionDoneTool.
